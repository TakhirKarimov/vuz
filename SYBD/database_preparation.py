...
def fill_z2(cur):
    filling_column_z2 = 'UPDATE НИР SET ВУЗ_кратко = (SELECT ВУЗы.ВУЗ_кратко) FROM ВУЗы WHERE НИР.код_ВУЗа = ВУЗы.код_ВУЗа'
...
def rename_columns(cur):
    new_columns_NIR = ['код_ВУЗа', 'ВУЗ_кратко', 'НТП', 'рег. номер', 'проект', 'код_ГРНТИ',
                       'руководитель', 'должность рук.', 'наличие экспаната', 'выставка', 'название экспаната']
    new_columns_VUZ = ['код_ВУЗа', 'ВУЗ', 'ВУЗ полный', 'ВУЗ_кратко', 'фед. округ', 'город',
...
path_to_xls_files = r"C:\Users\HP\Desktop\SYBD"
path_to_db = r'C:\Users\HP\Desktop\SYBD\SYBD.db'
